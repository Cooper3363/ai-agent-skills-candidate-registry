...
- intake_candidate: agency_agents_zh_role_library_candidate
- intake_type: 六部门 Agent 角色库候选源

## Current Decision

可以入候选库，但按角色库候选源处理，不按标准 SKILL.md / skill.yaml Skill 处理。当前只落源级候选卡，后续按 Top60 子角色筛选，不一次性把全仓角色都计入候选。

## Top60 Role Screening Queue
...
## License Window Requirements

- 确认 MIT 许可证和上游英文版版权边界。
- 检查 convert/install 脚本是否会写入本地工具目录，测试阶段必须禁止真实执行。
- 检查每个子角色是否包含真实 API、账号、文件写入、外部发送、法律/税务/财务/HR 高风险承诺。

## Test Bench Requirements

- 只对 L1 放行角色做 role smoke。
- 每个角色至少 1 个中文六部门岗位场景 mock。
- Role smoke 只验证角色说明能否产生稳定岗位输出，不等于 L2 通过。

## Boundary

不安装依赖，不执行脚本，不写 OpenClaw/Hermes/Codex 用户目录，不调用外部 API，不读取真实客户文件，不客户调用。稳交付库仍为 13。
