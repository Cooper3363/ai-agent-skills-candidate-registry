# License Review To150 Native Skills Queue

更新日期: 2026-06-03

## Scope

请许可证窗口复核 To150 新增 50 个候选。重点确认来源、许可证、商用限制、依赖/API/模型风险，以及是否可进入 DeepAgents smoke。

## Required L1 Output

每个候选必须输出: license_status, commercial_use_notes, upstream_skill_md_verified, upstream_manifest_verified, dependency_api_model_risk, trial_mode_final, l1_result。
...
许可证不清、无法定位具体 SKILL.md、聚合仓库无法定位子 skill、或依赖/API 条款不清的，不得进入 DeepAgents smoke。
