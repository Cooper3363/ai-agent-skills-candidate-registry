...
## 当前状态
- status: draft_l3
- source_candidate_id: quality_sprint08_unstructured_support_sop_parser_candidate
...
## 一句话调用意图
基于 mock SOP text rows 与 section rules 输出 SOP 分段、FAQ seed 和缺失章节，不读真实文件、不 OCR/上传、不生成 artifact。

## 来源项目与许可证 / L2 摘要
- source_project: Quality Sprint 08 / SaaS-readonly-mock candidate
- source_candidate_id: quality_sprint08_unstructured_support_sop_parser_candidate
- provider_or_system: Unstructured
...
- 上线前待办: 必须补真实 Harness smoke，并锁定最小授权 scope 或 mock 输入边界：mock text rows only; no file/OCR/upload/artifact。
...
- read_scope_required: mock text rows only; no file/OCR/upload/artifact
- mock_input_boundary: system=Unstructured, input_type=mock text rows only; no file/OCR/upload/artifact, no_real_file=true, no_upload=true, no_artifact=true, external_action_blocked=true
...
与 support_kb_doc_cleaner 和 contract_section_partitioner 复用文本分段，但本包仅处理 mock SOP text rows。

## 最小调用样例
输入 mock SOP text rows/section rules，输出 parsed_sections、faq_seeds、quality_warnings。

## 中文 mock/read-only smoke 用例
1. SOP 到 FAQ seed：mock SOP 文本含服务流程。预期 parsed_sections、faq_seeds、source_row_ids；失败为读真实文件。
2. 退货政策 SOP 分段：mock 文本含退货条件和例外。预期 parsed_sections、missing_sections、manual_review_required；失败为生成 artifact。
3. 客服新人手册大纲：mock 手册文本章节混杂。预期 quality_warnings、blocked_actions；失败为 OCR/上传。

## 失败模式
- 读取真实文件
- OCR 或上传
- 生成 PDF/HTML/artifact
- 缺少 source_row_ids
- 未触发人工复核
